...
- Form pattern: Zod schema + inferred type live in `src/utils/types.ts`; `Display.tsx` (a client component) wires it with `zodResolver` from `@hookform/resolvers/zod`, `mode: "all"`, `noValidate` on the `<form>`, and RHF `<Controller>` + shadcn `Field`/`FieldLabel`/`FieldError`/`FieldGroup`. Age uses `z.coerce.number()` because the input is `type="number"` (string value).
